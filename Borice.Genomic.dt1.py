# b1 :: beta version 1
# dt1 :: deals with input datatype 1
...
GProb={"RR":{},"RA":{},"AA":{}} # Genotyping probilities:: These are overwritten by data for data type I
			...
			for MG in range(3):
				...
				for offspring in range(1,Famsize):					
					if delt[offspring-1]==0:
						L0 = OProb[MG][0][0]*Data[offspring][0] + OProb[MG][0][1]*Data[offspring][1] + OProb[MG][0][2]*Data[offspring][2] # dad gives R
						L1 = OProb[MG][1][0]*Data[offspring][0] + OProb[MG][1][1]*Data[offspring][1] + OProb[MG][1][2]*Data[offspring][2] # dad gives A
						Fk*= (q*L0+(1-q)*L1)

						# print "zz ",L0,L1
					elif delt[offspring-1]==1:
						Fk*= ( SProb[MG][0]*Data[offspring][0] + SProb[MG][1]*Data[offspring][1] + SProb[MG][2]*Data[offspring][2] )
	...
	for j in range(1,NoFamilies+1):
		for offspring in range(1,len(FullData[NamedSNP][j])):
			...
			if deltp[offspring-1]==1:
				PriorRatio=1.0/PriorRatio # step to selfed
...
def GenoPP(): # Posterior probabilities for genotypes
	for j in range(1,NoFamilies+1):
		...
		for snpID in Q[0]:
			...
			if dg==0:
				pass # no data for family

			elif dg==1 and Famsize==1: # only a mom that has data (she produced no genotyped progeny)
				
				PostMom[0]=PriorMom[0]*Data[0][0]
				PostMom[1]=PriorMom[1]*Data[0][1]
				PostMom[2]=PriorMom[2]*Data[0][2]
				for ma in range(3):
				...
				for offspring in range(1,Famsize):
					if delt[offspring-1]==0:
						PostDad=[0.0,0.0]
						for MG in range(3):
							PostDad[0]+=PostMom[MG]*(OProb[MG][0][0]*Data[offspring][0] + OProb[MG][0][1]*Data[offspring][1] + OProb[MG][0][2]*Data[offspring][2]) # dad gives R
							PostDad[1]+=PostMom[MG]*(OProb[MG][1][0]*Data[offspring][0] + OProb[MG][1][1]*Data[offspring][1] + OProb[MG][1][2]*Data[offspring][2])
...
t ={0:0.5} # initial value for population outcrossing rate
...
# 82	off	1e-05	0.0625	1.0
	if cols[0][0]==NamedSNP[0]:
		...
		FullData[snpID][int(cols[0])].append([float(cols[2]), float(cols[3]), float(cols[4])]) # first list element is mom
...
for steps in range(ChainLength):
	...
	if steps % thinningfreq == 0:
		...
		cLL=FLL()
                out4.write(str(steps)+'\t'+str(t[0])+'\t'+str(cLL)+'\n')
...
stx=["Fpop","t","delta","IH","Qancestral","Qsubpop"]
for j in range(6):
